...
//*********step1:JD详情页-左右点击拨动图片功能*********
	checkA:function(){//检查两个a的状态
		...
		}else if(this.moved==this.MAX){//否则，如果moved等于MAX
      		//设置$for的class为forward_disabled
      		$for.attr("class","forward_disabled");
		}else{//否则
			$back.attr("class","backward");//$back的class为backward
			$for.attr("class","forward");//$for的class为forward
		}
	},
//*********step2:JD详情页-鼠标滑动小图片，上面显示大图片*********
